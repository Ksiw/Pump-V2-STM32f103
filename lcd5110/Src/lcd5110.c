...
void LCD5110_wline(int x1, int y1, int x2, int y2, int color,
	...
	for (int i = xb; i <= xe; ++i)
	    ...
	    if (i >= win->frame.x0 && i < win->frame.x0 + win->frame.width
		    && cury >= win->frame.y0
		    && cury < win->frame.y0 + win->frame.height)
		{
	...
	for (int i = yb; i <= ye; ++i)
	    ...
	    if (curx >= win->frame.x0 && curx < win->frame.x0 + win->frame.width
		    && i >= win->frame.y0
		    && i < win->frame.y0 + win->frame.height)
		{
...
void LCD5110_wrect(rect_t* rect, int color, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
    LCD5110_wline(rect->x0, rect->y0, rect->x0 + rect->width - 1, rect->y0,
	    color, win, lcd_conf);
    LCD5110_wline(rect->x0, rect->y0 + rect->height - 1,
	    rect->x0 + rect->width - 1, rect->y0 + rect->height - 1, color, win,
	    lcd_conf);
    LCD5110_wline(rect->x0, rect->y0, rect->x0, rect->y0 + rect->height - 1,
	    color, win, lcd_conf);
...
void LCD5110_win_draw_frame(int color, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
...
void LCD5110_wrect_fill(rect_t* rect, int color, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
    //! Slow!
	...
	for (int j = win->frame.y0 + rect->y0; j < win->frame.y0 + down - 1;
		++j)
	    LCD5110_putpix(i, j, color, &lcd_conf->hw_conf);
    }

void LCD5110_putchar(int px, int py, char ch, int color,
	LCD5110_display* lcd_conf)
    {
...
void LCD5110_wputchar(int px, int py, char ch, int color, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
...
//! TODO: Print part of char
    const unsigned char *fontpointer = base_font;
    LCD5110_drawBitmap(px + win->frame.x0, py + win->frame.y0,
	    fontpointer + ch * base_font_width, base_font_width,
	    base_font_height, !color, &lcd_conf->hw_conf);
    }
...
void LCD5110_clear_char(int x1, int y1, int color, LCD5110_display* lcd_conf)
    ...
    rect_t rect =
	{
	x1, y1, x1 + base_font_width, y1 + base_font_height
	};
    LCD5110_rect_fill(&rect, color, lcd_conf);
    }

void LCD5110_wclear_char(int x1, int y1, int color, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
    //! Stupid
    rect_t rect =
...
void LCD5110_wclear_str(int x, int y, int len, int color, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
    for (int i = 0; i < len; ++i)
	{
...
void LCD5110_clear_str(int x, int y, int len, int color,
	LCD5110_display* lcd_conf)
    {
    LCD5110_wclear_str(x, y, len, color, &lcd_conf->def_scr, lcd_conf);
    }

void LCD5110_wset_cursor(int x, int y, LCD5110_canvas_t* win,
	LCD5110_display* lcd_conf)
    {
    win->cursor.x = x;
    win->cursor.y = y;
...
void LCD5110_wprint(const char* str, int color, LCD5110_canvas_t* win,
	...
	if (*str != '\n')
	    {
	    LCD5110_wputchar(win->cursor.x, win->cursor.y, *str, color, win,
		    lcd_conf);
	    }
...
int LCD5110_wprintf(LCD5110_display* lcd_conf, int color, LCD5110_canvas_t* win,
	char *fmt, ...)
    {
    char buf[100];
    int res;
