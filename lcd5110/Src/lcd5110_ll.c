...
static inline HAL_StatusTypeDef send_byte_to_LCD5110(uint8_t dat,
	LCD5110_conf* lcd_conf)
    {
...
static inline HAL_StatusTypeDef send_data_to_LCD5110(uint8_t data[],
	uint16_t size, LCD5110_conf* lcd_conf)
    {
...
static inline void LCD5110_CE_on(LCD5110_conf* lcd_conf)
    ...
    HAL_GPIO_WritePin(lcd_conf->spi_cs_port, lcd_conf->spi_cs_pin,
	    GPIO_PIN_SET);
    }
...
//! H==0
static int LCD5110_set_mode_base(LCD5110_modes mode_byte,
	LCD5110_conf* lcd_conf)
    {
...
static inline int LCD5110_set_XY_base(int16_t x, int16_t y,
	LCD5110_conf* lcd_conf)
    {
    int res;
...
int LCD5110_init(LCD5110_conf* lcd_conf, LCD5110_modes dmode, uint8_t voltage,
	uint8_t temp_coeff, uint8_t bias)
    {
    SPI_enable(lcd_conf);
...
void LCD5110_refresh_ll(LCD5110_conf* lcd_conf)
    ...
    send_data_to_LCD5110(lcd_conf->video_buffer, LCD_HEIGHT * LCD_WIDTH / 8,
	    lcd_conf);
//!TODO: Check for transmission end and turn CE off (set it to HIGH).
    }
